...
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    const xTo = window.gsap.quickTo(cursorRef.current, "x", {duration: 0.8, ease: "elastic.out(1, 0.3)"});
    const yTo = window.gsap.quickTo(cursorRef.current, "y", {duration: 0.8, ease: "elastic.out(1, 0.3)"});
    const xDotTo = window.gsap.quickTo(dotRef.current, "x", {duration: 0.1, ease: "power3"});
    const yDotTo = window.gsap.quickTo(dotRef.current, "y", {duration: 0.1, ease: "power3"});
  ...
  return (
    <>
      <div ref={cursorRef} className="hidden md:block fixed top-0 left-0 w-8 h-8 border-2 border-black rounded-full pointer-events-none z-[100] -translate-x-1/2 -translate-y-1/2 mix-blend-difference" />
      <div ref={dotRef} className="hidden md:block fixed top-0 left-0 w-2 h-2 bg-[#FFDF00] rounded-full pointer-events-none z-[100] -translate-x-1/2 -translate-y-1/2 mix-blend-difference" />
    </>
  );
};
...
const AnimatedBackground = () => {
  ...
  useEffect(() => {
    ...
    gsap.to(shape2Ref.current, {
      ...
      rotation: -45,
      ease: "none",
      scrollTrigger: { trigger: document.body, start: "top top", end: "bottom top", scrub: 2 }
    });
  }, [gsap]);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[-1] overflow-hidden bg-[#f4f4f0]">
      <div className="absolute inset-0 opacity-[0.1]" 
           style={{ backgroundImage: 'radial-gradient(black 2px, transparent 2px)', backgroundSize: '30px 30px', animation: 'panGrid 20s linear infinite' }} 
      />
      <div ref={shape1Ref} className="absolute top-20 right-10 text-[10rem] font-black text-[#A6FAFF] opacity-30 select-none leading-none">*</div>
      <div ref={shape2Ref} className="absolute bottom-[-10%] left-[10%] w-64 h-64 border-[20px] border-[#FF90E8] opacity-20 rounded-full" />
      <style>{`
...
const BootSequence = ({ onComplete }) => {
  ...
  useEffect(() => {
    const sequences = ['LOADING GSAP MODULES', 'BYPASSING TURNITIN', 'COMPILING THESIS', 'SYSTEM READY_'];
    let i = 0;
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black text-[#FFDF00] flex flex-col items-center justify-center font-mono animate-[slideUp_0.8s_ease-in-out_2s_forwards] px-4 text-center">
      <div className="text-3xl md:text-5xl font-black mb-6 animate-pulse">JOKIHUB.EXE</div>
...
const NeoCard = ({ children, className = "", color = "bg-white", onClick }) => {
  ...
  const handleMouseLeave = () => {
    ...
    gsap.to(cardRef.current, { rotationY: 0, rotationX: 0, boxShadow: "4px 4px 0px 0px rgba(0,0,0,1)", ease: "elastic.out(1, 0.3)", duration: 1 });
  };
...
const NeoButton = ({ children, onClick, className = "", color = "bg-[#FFDF00]", icon: Icon, disabled = false }) => (
  <button
    disabled={disabled}
    onClick={onClick}
...
const NeoBadge = ({ children, color = "bg-[#FF90E8]", className="" }) => (
  <span className={`px-2 py-1 md:px-3 md:py-1 text-[10px] md:text-xs font-black uppercase border-2 border-black ${color} shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 transition-all cursor-default ${className}`}>
    {children}
  </span>
);
...
const Marquee = ({ text }) => (
  <div className="w-full overflow-hidden border-y-2 md:border-y-4 border-black bg-[#FFDF00] py-2 md:py-3 flex items-center relative z-10 shadow-[0_4px_0_0_rgba(0,0,0,1)] md:shadow-[0_6px_0_0_rgba(0,0,0,1)]">
    <div className="flex whitespace-nowrap animate-[marquee_20s_linear_infinite]">
      {[...Array(8)].map((_, i) => (
        <span key={i} className="mx-4 md:mx-6 text-sm md:text-lg font-black uppercase tracking-widest flex items-center gap-4 hover:text-white transition-colors duration-300">
          {text} <Zap className="w-4 h-4 md:w-6 md:h-6 fill-black hover:fill-white transition-colors" />
        </span>
      ))}
    </div>
...
const OrderCalculator = () => {
  ...
  const handleOrder = async () => {
    ...
    if (service === 'parafrase' && (Number(currentPlagiasi) <= Number(targetPlagiasi) || currentPlagiasi === '' || targetPlagiasi === '')) {
      return toast('Persentase plagiasi awal harus lebih besar dari target!', 'error');
    }
  ...
  return (
    <section id="kalkulator" className="py-16 md:py-24 px-4 md:px-8 bg-[#3b82f6] text-white border-y-4 md:border-y-8 border-black relative overflow-hidden" ref={calcRef}>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-12 items-start relative z-10">
        <div className="space-y-6 md:space-y-8">
          <SplitTextReveal text="KALKULATOR SAKTI" className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#FFDF00] drop-shadow-[4px_4px_0_rgba(0,0,0,1)]" triggerRef={calcRef} />
          ...
          <div className="hidden lg:block relative mt-8 group cursor-default max-w-sm">
            ...
            <div className="relative z-10 border-4 border-black bg-white text-black p-6 transform transition-transform group-hover:-translate-y-1 group-hover:-translate-x-1">
              ...
              <ul className="space-y-3 font-bold text-sm">
                <li className="flex items-center gap-3"><CheckCircle2 className="text-green-500 w-5 h-5 shrink-0"/> Bebas Plagiasi & Lolos Turnitin</li>
                <li className="flex items-center gap-3"><CheckCircle2 className="text-green-500 w-5 h-5 shrink-0"/> Garansi Revisi Sampai ACC</li>
                <li className="flex items-center gap-3"><CheckCircle2 className="text-green-500 w-5 h-5 shrink-0"/> Privasi Data Klien 100% Aman</li>
              </ul>
            </div>
          </div>
        </div>
...
const OrderTracker = () => {
  ...
  const getStatusInfo = (status) => {
    const map = {
      'pending': { text: 'MENUNGGU PEMBAYARAN', color: 'bg-[#FFDF00]', icon: Clock },
      'process': { text: 'SEDANG DIKERJAKAN', color: 'bg-[#A6FAFF]', icon: Settings },
      'done': { text: 'SELESAI!', color: 'bg-[#90EE90]', icon: CheckCircle2 },
    };
    return map[status] || { text: 'TIDAK DIKETAHUI', color: 'bg-gray-200', icon: AlertCircle };
  };

  return (
    <section id="lacak" className="py-16 md:py-24 px-4 md:px-8 max-w-4xl mx-auto relative" ref={trackRef}>
      <div className="text-center mb-10 md:mb-12">
        <SplitTextReveal text="LACAK PESANAN" className="text-3xl sm:text-4xl lg:text-5xl font-black uppercase mb-4 justify-center" triggerRef={trackRef} />
      ...
      {hasSearched && (
        <div className="animate-[slideIn_0.4s_cubic-bezier(0.175,0.885,0.32,1.275)]">
          {trackedOrder ? (
            <NeoCard color="bg-black" className="text-white transform rotate-1 hover:rotate-0 p-5 md:p-8">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b-2 border-gray-700 pb-5 mb-5 gap-4">
                <div>
                  <p className="font-bold uppercase text-[#FF90E8] mb-1 text-xs tracking-widest">Detail Pesanan Ditemukan</p>
                  <h3 className="text-2xl md:text-3xl font-black text-[#A6FAFF]">{trackedOrder.id}</h3>
                </div>
                {(() => {
                  const Info = getStatusInfo(trackedOrder.status);
                  return (
                    <div className={`flex items-center gap-2 border-2 md:border-4 border-black ${Info.color} text-black p-2 md:p-3 shadow-[3px_3px_0px_0px_#FFDF00]`}>
                      <Info.icon className="w-5 h-5 md:w-6 md:h-6" />
                      <span className="font-black text-sm md:text-base uppercase">{Info.text}</span>
                    </div>
                  );
                })()}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 font-bold text-sm md:text-base">
                <div className="bg-gray-900 p-4 border-l-4 border-[#FFDF00]">
                  <p className="text-gray-400 uppercase text-xs mb-1">Layanan</p>
                  <p>{trackedOrder.service}</p>
                </div>
                <div className="bg-gray-900 p-4 border-l-4 border-[#A6FAFF]">
                  <p className="text-gray-400 uppercase text-xs mb-1">Pemesan</p>
                  <p>{trackedOrder.client}</p>
                </div>
                <div className="bg-gray-900 p-4 border-l-4 border-[#FF90E8]">
                  <p className="text-gray-400 uppercase text-xs mb-1">Tanggal Order</p>
                  <p>{trackedOrder.date}</p>
                </div>
                <div className="bg-gray-900 p-4 border-l-4 border-[#90EE90]">
                  <p className="text-gray-400 uppercase text-xs mb-1">Total Pembayaran</p>
                  <p className="text-xl md:text-2xl font-black">{formatRupiah(trackedOrder.amount)}</p>
                </div>
              </div>
            </NeoCard>
          ) : (
            <NeoCard color="bg-[#FFDF00]" className="text-center py-10 animate-bounce">
              <AlertCircle className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-4" />
              <h3 className="text-xl md:text-2xl font-black uppercase mb-2">WADUH, TIDAK KETEMU!</h3>
              <p className="font-bold text-sm md:text-base">Pastikan ID Pesanan diketik dengan benar (Contoh: ORD-1234).</p>
            </NeoCard>
          )}
        </div>
      )}
    ...
    <div className="min-h-screen text-black font-sans selection:bg-[#FFDF00] selection:text-black overflow-x-hidden relative">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white border-b-4 border-black shadow-[0_4px_0_0_rgba(0,0,0,1)]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2 md:gap-3 cursor-pointer group select-none" onClick={handleLogoClick}>
            ...
            <span className="font-black text-lg md:text-2xl tracking-tighter">JOKIHUB.</span>
          </div>
          <div className="hidden md:flex gap-6 font-black uppercase text-xs md:text-sm tracking-widest">
            <a href="#layanan" className="hover:text-[#FF90E8] hover:-translate-y-0.5 transition-all">Layanan</a>
            <a href="#kalkulator" className="hover:text-[#3b82f6] hover:-translate-y-0.5 transition-all">Order</a>
            <a href="#lacak" className="hover:text-[#90EE90] hover:-translate-y-0.5 transition-all">Lacak</a>
          </div>
        </div>
      </nav>
      ...
      <section ref={heroRef} className="pt-24 md:pt-32 pb-16 md:pb-24 px-4 md:px-8 max-w-6xl mx-auto flex flex-col lg:flex-row items-center gap-8 md:gap-12 relative">
        <div className="flex-1 space-y-5 md:space-y-6 z-10 mt-8 md:mt-0">
          ...
          <SplitTextReveal text="TUGAS KELAR, HATI AMBYAR (EH, TENANG!)." className="text-4xl sm:text-5xl lg:text-6xl font-black leading-[1.1] uppercase" />
          <p className="text-base md:text-lg lg:text-xl font-bold border-l-4 md:border-l-8 border-black pl-3 md:pl-5 py-2 bg-white/80 md:bg-white/50 backdrop-blur-sm max-w-xl">
            Solusi brutal untuk tugas akhirmu. Skripsi, parafrase, tugas harian. Serahkan ke ahlinya, kamu tinggal tidur.
          </p>
          <div className="flex flex-wrap gap-3 md:gap-4 pt-2">
            <NeoButton onClick={() => document.getElementById('kalkulator').scrollIntoView({behavior: 'smooth'})} icon={ArrowRight} color="bg-[#FF90E8]" className="text-sm md:text-base w-full sm:w-auto">
        ...
        <div className="flex-1 relative w-full aspect-square max-w-sm lg:max-w-md group">
          ...
          <div className="absolute -bottom-6 -left-6 md:-bottom-8 md:-left-8 z-20 animate-spin-slow">
            <div className="w-20 h-20 md:w-24 md:h-24 bg-[#A6FAFF] border-4 md:border-8 border-black rounded-full flex items-center justify-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              <span className="font-black text-center text-sm md:text-base leading-tight">100%<br/>PRO</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <ScrollStagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {SERVICES.map((service, index) => (
            <NeoCard key={index} color={service.color} className="flex flex-col h-full !p-5 md:!p-6">
              ...
              <h3 className="text-lg md:text-xl font-black uppercase mb-3 leading-tight">{service.title}</h3>
              <p className="font-bold text-sm md:text-base mb-5 md:mb-6 flex-grow">Mulai <br/><span className="text-lg md:text-xl font-black bg-white px-2 mt-1 inline-block border-2 border-black">{formatRupiah(service.basePrice)}</span></p>
              <NeoButton color="bg-white" className="w-full !py-2 md:!py-2.5 text-sm md:text-base group-hover:bg-black group-hover:text-white" onClick={() => document.getElementById('kalkulator').scrollIntoView({behavior: 'smooth'})}>
                GAS PILIH
              </NeoButton>
            </NeoCard>
          ))}
      ...
      <footer className="bg-black text-white py-10 md:py-12 border-t-[8px] md:border-t-[12px] border-[#FFDF00] relative z-20">
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-left">
            <h2 className="text-3xl md:text-4xl font-black tracking-tighter mb-2">JOKIHUB.</h2>
            <p className="font-bold text-sm md:text-base text-gray-400 max-w-sm mb-3">Menyelesaikan masalah akademis Anda dengan gaya sejak 2026.</p>
          </div>
...
const AdminDashboard = ({ onLogout }) => {
  ...
  const getStatusBadge = (status) => {
    ...
    return <NeoBadge color={map[status].color} className="text-xs px-2 py-1 md:px-3 md:py-1.5">{map[status].label}</NeoBadge>;
  };
  ...
  const Sidebar = () => (
    <div className={`fixed inset-y-0 left-0 z-[150] w-64 md:w-64 bg-white border-r-4 md:border-r-8 border-black transform transition-transform duration-300 ease-in-out flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:relative`}>
      ...
      <div className="p-4 md:p-5 border-t-4 md:border-t-8 border-black bg-black">
        <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 font-black uppercase py-2.5 md:py-3 bg-white border-2 md:border-4 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-all text-sm md:text-base">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 md:h-20 bg-white border-b-4 md:border-b-8 border-black flex items-center justify-between px-4 md:px-8 shrink-0 z-10 shadow-[0_4px_0_0_rgba(0,0,0,1)]">
          ...
          <div className="flex items-center gap-3">
            <span className="font-black text-xs md:text-sm hidden sm:block bg-[#FFDF00] border-2 border-black px-3 py-1.5 shadow-[2px_2px_0_0_rgba(0,0,0,1)]">ADMIN UTAMA</span>
          </div>
        </header>
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-6 md:space-y-8 animate-[slideIn_0.4s_ease-out] max-w-6xl">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                <NeoCard color="bg-[#A6FAFF]" className="!p-5 hover:-translate-y-1">
                ...
                <NeoCard color="bg-[#FFDF00]" className="!p-5 hover:-translate-y-1">
                  <div className="flex justify-between items-start">
                    <div>
                ...
                <NeoCard color="bg-[#90EE90]" className="!p-5 hover:-translate-y-1">
                  <div className="flex justify-between items-start">
                    <div>
                  ...
                  <table className="w-full text-left border-collapse min-w-[700px]">
                    <thead>
                      <tr className="bg-black text-white uppercase text-xs md:text-sm font-black tracking-widest">
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">ID</th>
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">Klien</th>
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">Layanan</th>
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {orders.slice(0, 5).map((order, i) => (
                        <tr key={order.id} className={`font-bold text-xs md:text-sm border-b-2 md:border-b-4 border-black transition-colors ${i % 2 === 0 ? 'bg-white hover:bg-gray-100' : 'bg-[#f4f4f0] hover:bg-gray-200'}`}>
                          <td className="p-3 md:p-4 border-r-2 md:border-r-4 border-black font-black">{order.id}</td>
                          <td className="p-3 md:p-4 border-r-2 md:border-r-4 border-black">{order.client}</td>
                          <td className="p-3 md:p-4 border-r-2 md:border-r-4 border-black">{order.service}</td>
              ...
              <div className="border-4 md:border-8 border-black bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse min-w-[900px]">
                    <thead>
                      <tr className="bg-black text-white uppercase text-xs md:text-sm font-black tracking-widest">
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">ID & Tgl</th>
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">Klien & WA</th>
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">Layanan & Catatan</th>
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">Total</th>
                        <th className="p-3 md:p-4 border-r-2 md:border-r-4 border-gray-700">Status</th>
